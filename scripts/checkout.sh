...
then
	...
	then
		clear
		read -p "Are changelogs and READMEs up to date? [Y/n] " -n 1 -r
		echo -e "\n"  # move to a new line
		if [[ $REPLY =~ ^[Yy]$ ]]
		then
			clear
			read -p "Are all new classes added to the NeedleJS namespace? [Y/n] " -n 1 -r
			echo -e "\n"  # move to a new line
			if [[ $REPLY =~ ^[Yy]$ ]]
			then
				clear
				...
				then
				    clear
					read -p "Is the new version correctly updated in the package.json, and readme? [Y/n] " -n 1 -r
					echo -e "\n"  # move to a new line
					if [[ $REPLY =~ ^[Yy]$ ]]
					then
						clear
